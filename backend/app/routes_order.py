...
# Frontend pyta o to, zamiast trzymac flagi na sztywno w JS.
# FIRM_ORDERS_ENABLED=false => ukryte pola NIP/faktura i przycisk "Firma" w /zamow.
@router.get("/api/config/features")
def get_features():
    return {
        "ok": True,
        "firm_orders": bool(getattr(settings, "FIRM_ORDERS_ENABLED", False)),
    }
...
<p class=muted>Dokument nie jest fakturą VAT w rozumieniu przepisów — sprzedaż okazjonalna osoby fizycznej.</p>
<!-- FIRMA OFF (KSeF/NIP od 1.04.2026) - wylaczone do czasu NIP/JDG. Oryginalna linia: Dla firm: dane NIP w zamówieniu: {('NIP w uwagach' if (o.notes and 'NIP' in (o.notes or '')) else 'brak')}.</p> -->
